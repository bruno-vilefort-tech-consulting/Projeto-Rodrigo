...
const ContactSchema = Yup.object().shape({
  name: Yup.string()
    .min(2, i18n.t("flowBuilderConfig.validation.tooShort"))
    .max(50, i18n.t("flowBuilderConfig.validation.tooLong"))
    .required(i18n.t("flowBuilderConfig.validation.enterName")),
  text: Yup.string()
    .min(2, i18n.t("flowBuilderConfig.validation.tooShort"))
    .max(50, i18n.t("flowBuilderConfig.validation.tooLong"))
    .required(i18n.t("flowBuilderConfig.validation.enterMessage"))
});
...
const FlowBuilderConditionModal = ({ open, onSave, onUpdate, data, close }) => {
  ...
  const [labels, setLabels] = useState({
    title: i18n.t("flowBuilderConfig.modals.condition.addTitle"),
  ...
  useEffect(() => {
    if (open === "edit") {
      setLabels({
        title: i18n.t("flowBuilderConfig.modals.condition.editTitle"),
    ...
    <div className={classes.root}>
      ...
      >
        ...
        <Stack>
          ...
          >
            <TextField
              label={i18n.t("flowBuilderConfig.modals.condition.fieldLabel")}
              rows={7}
              name="text"
              variant="outlined"
              value={textDig}
            ...
            <FormControl sx={{ width: "95%" }} size="medium">
              <InputLabel sx={selectFieldStyles} id="demo-simple-select-label">
                {i18n.t("flowBuilderConfig.modals.condition.validationRule")}
              </InputLabel>
              <Select
                labelId="demo-simple-select-label"
                id="demo-simple-select"
                value={rule}
                label={i18n.t("flowBuilderConfig.modals.condition.validationRule")}
            ...
            <TextField
              label={i18n.t("flowBuilderConfig.modals.condition.conditionValue")}
              rows={7}
              name="text"
              variant="outlined"
              value={valueCondition}
