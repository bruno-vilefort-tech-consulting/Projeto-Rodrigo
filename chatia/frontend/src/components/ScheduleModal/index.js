...
const ScheduleSchema = Yup.object().shape({
	body: Yup.string()
		.min(5, i18n.t("scheduleModal.validations.tooShort"))
		.required(i18n.t("scheduleModal.validations.required")),
	contactId: Yup.number().required(i18n.t("scheduleModal.validations.required")),
	sendAt: Yup.string().required(i18n.t("scheduleModal.validations.required"))
});
		...
		<div className={classes.root}>
			...
			>
				<DialogTitle id="form-dialog-title">
					{schedule.status === 'ERRO' ? i18n.t("scheduleModal.status.error") : `${i18n.t("scheduleModal.status.sending")} ${capitalize(schedule.status)}`}
				</DialogTitle>
					...
					{({ touched, errors, isSubmitting, values, setFieldValue }) => (
						<Form>
							<DialogContent dividers>
								...
								<h3>{i18n.t("scheduleModal.recurrence.title")}</h3>
								<p>
									{i18n.t("scheduleModal.recurrence.description")}
								</p>
								<br />
								<Grid container spacing={1}>
									<Grid item xs={12} md={4} xl={4}>
										<FormControl size="small" fullWidth variant="outlined">
											<InputLabel id="demo-simple-select-label">{i18n.t("scheduleModal.recurrence.interval")}</InputLabel>
											<Select
												...
												label={i18n.t("scheduleModal.recurrence.interval")}
											>
												<MenuItem value={1}>{i18n.t("scheduleModal.recurrence.intervalTypes.days")}</MenuItem>
												<MenuItem value={2}>{i18n.t("scheduleModal.recurrence.intervalTypes.weeks")}</MenuItem>
												<MenuItem value={3}>{i18n.t("scheduleModal.recurrence.intervalTypes.months")}</MenuItem>
												<MenuItem value={4}>{i18n.t("scheduleModal.recurrence.intervalTypes.minutes")}</MenuItem>
											</Select>
										</FormControl>
									</Grid>
									...
									<Grid item xs={12} md={4} xl={4}>
										<Field
											as={TextField}
											label={i18n.t("scheduleModal.recurrence.intervalValue")}
											name="valorIntervalo"
											size="small"
									...
									<Grid item xs={12} md={4} xl={4}>
										<Field
											as={TextField}
											label={i18n.t("scheduleModal.recurrence.sendTimes")}
									...
									<Grid item xs={12} md={12} xl={12}>
										<FormControl size="small" fullWidth variant="outlined">
											<InputLabel id="demo-simple-select-label">{i18n.t("scheduleModal.recurrence.sendTimes")}</InputLabel>
											<Select
												labelId="demo-simple-select-label"
												id="demo-simple-select"
												value={tipoDias}
												...
												label={i18n.t("scheduleModal.recurrence.sendTimes")}
											>
												<MenuItem value={4}>{i18n.t("scheduleModal.recurrence.businessDays.normal")}</MenuItem>
												<MenuItem value={5}>{i18n.t("scheduleModal.recurrence.businessDays.before")}</MenuItem>
												<MenuItem value={6}>{i18n.t("scheduleModal.recurrence.businessDays.after")}</MenuItem>
											</Select>
										</FormControl>
									</Grid>
								</Grid>
