  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (!planConfigs.plan.useCampaigns) {
        toast.error(i18n.t("campaignReport.noPermission"));
        setTimeout(() => {
          history.push(`/`)
        }, 1000);
      }
    ...
    <MainContainer>
      ...
      <Paper className={classes.mainPaper} variant="outlined">
        <Typography variant="h6" component="h2">
          {i18n.t("campaignReport.status")} {formatStatus(campaign.status)} {delivered} {i18n.t("campaignReport.of")} {validContacts}
        </Typography>
