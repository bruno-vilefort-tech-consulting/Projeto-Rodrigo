            ...
            <Grid spacing={1} justifyContent="center" container>
              ...
              <Grid xs={12} sm={6} md={2} item>
                <FormControl margin="dense" variant="outlined" fullWidth>
                  ...
                  <Field
                    as={Select}
                    label={i18n.t("settings.managers.companies.recurrence")}
...
export function CompaniesManagerGrid(props) {
  ...
  const { formatCurrency, currency } = useCurrencyContext();

  const renderStatus = (row) => {
    return row.status === false ? i18n.t("settings.managers.common.no") : i18n.t("settings.managers.common.yes");
  };
  ...
  const renderPlanValue = (row) => {
    if (row.planId !== null && row.plan !== null && row.plan.amount) {
      return formatCurrency(row.plan.amount);
    }
...
export default function CompaniesManager() {
  ...
  const handleSubmit = async (data) => {
    ...
    try {
      ...
      toast.error(
        i18n.t("settings.toasts.companyOperationError")
      );
    }
    setLoading(false);
  };
