...
#=========================== OTIMIZAÇÃO TMPFS ==================================
# Carregar sistema de otimização tmpfs
if [ -f "/root/tmpfs-optimization.sh" ]; then
    source /root/tmpfs-optimization.sh
    TMPFS_ENABLED=1
else
    TMPFS_ENABLED=0
    echo "[AVISO] Sistema tmpfs não encontrado - executando sem otimização"
fi
  ...
  case "$LANG_UI" in
    pt)
      case "$k" in
        ...
        q_sql_backup) echo "(Opcional) Deseja importar um backup SQL? (S/N)";;
        q_sql_path) echo "Informe o caminho completo do arquivo .sql";;
        q_confirm) echo "Confirmar configuração? (S/N)";;

        err_empty) echo "Este campo é obrigatório.";;
        err_sql_not_found) echo "Arquivo SQL não encontrado.";;
        err_sql_invalid) echo "Arquivo deve ter extensão .sql";;
        ...
        importing_backup) echo "Importando backup SQL";;
        updating_admin) echo "Atualizando credenciais do admin";;
    ...
    es)
      case "$k" in
        ...
        q_sql_backup) echo "(Opcional) ¿Desea importar un backup SQL? (S/N)";;
        q_sql_path) echo "Ingrese la ruta completa del archivo .sql";;
        q_confirm) echo "¿Confirmar configuración? (S/N)";;

        err_empty) echo "Este campo es obligatorio.";;
        err_sql_not_found) echo "Archivo SQL no encontrado.";;
        err_sql_invalid) echo "El archivo debe tener extensión .sql";;
        ...
        importing_backup) echo "Importando backup SQL";;
        updating_admin) echo "Actualizando credenciales del admin";;
    ...
    *)
      case "$k" in
        ...
        q_sql_backup) echo "(Optional) Import a SQL backup? (Y/N)";;
        q_sql_path) echo "Enter the full path to the .sql file";;
        q_confirm) echo "Confirm configuration? (Y/N)";;

        err_empty) echo "This field is required.";;
        err_sql_not_found) echo "SQL file not found.";;
        err_sql_invalid) echo "File must have .sql extension";;
        ...
        importing_backup) echo "Importing SQL backup";;
        updating_admin) echo "Updating admin credentials";;
        done_ok) echo "Done";;
...
repo_url=${repo_url}
sql_backup_path=${sql_backup_path}
...
# Função hash para calcular portas (replica a lógica do ecosystem.config.js)
hash_port() {
  local name="$1"
  local base="$2"
  local h=0
...
coletar_dados_usuario() {
  ...
  while true; do
    case "$step" in
      ...
      ;;
      5)
        header_step "$step" "$total" "$(t q_deploy_pass)"
        echo -e "${GRAY_LIGHT}(senha visível - digite com cuidado)${NC}"
        read -r -p "> " inpt || true
      ...
      ;;
      6)
        header_step "$step" "$total" "$(t q_master_pass)"
        echo -e "${GRAY_LIGHT}(senha visível - digite com cuidado)${NC}"
        read -r -p "> " inpt || true
      ...
      12)
        header_step "$step" "$total" "$(t q_sql_backup)"
        read -rp "> " inpt || true
        case "${inpt^^}" in B) step=$((step-1)); continue;; R) unset sql_backup_path; step=1; continue;; Q) echo -e "${YELLOW}$(t cancelled)${NC}"; exit 1;; esac
        if yes_input "$inpt"; then step=$((step+1)); else sql_backup_path=""; step=$((step+2)); fi
      ;;
      13)
        header_step "$step" "$total" "$(t q_sql_path)"
        [ -n "${sql_backup_path:-}" ] && echo -e "${GRAY_LIGHT}(${sql_backup_path})${NC}"
        read -rp "> " inpt || true
        case "${inpt^^}" in B) step=$((step-1)); continue;; R) unset sql_backup_path; step=12; continue;; Q) echo -e "${YELLOW}$(t cancelled)${NC}"; exit 1;; esac
        [ -z "$inpt" ] && [ -n "${sql_backup_path:-}" ] && inpt="$sql_backup_path"
        [ -z "$inpt" ] && { echo -e "${RED}$(t err_empty)${NC}"; sleep 1; continue; }
        [[ "$inpt" =~ \.sql$ ]] || { echo -e "${RED}$(t err_sql_invalid)${NC}"; sleep 1; continue; }
        [ ! -f "$inpt" ] && { echo -e "${RED}$(t err_sql_not_found)${NC}"; sleep 1; continue; }
        sql_backup_path="$inpt"; step=$((step+1))
      ;;
      14)
        banner
        ...
        backend_port=$(hash_port "$empresa" 8000)
        frontend_port=$(hash_port "$empresa" 3001)
...
#========================= INICIALIZAÇÃO TMPFS =================================
if [ "$TMPFS_ENABLED" = "1" ]; then
    init_tmpfs_optimization
fi

#------------------- ETAPA 1: apt-cacher-ng ------------------------------------
[ "$TMPFS_ENABLED" = "1" ] && optimize_etapa1_aptcacher
...
[ "$TMPFS_ENABLED" = "1" ] && optimize_etapa2_3_apt
...
run_quiet "$(t configuring) user 'deploy'..." "
  # Criar grupo deploy se não existir
  if ! getent group deploy >/dev/null 2>&1; then
    /usr/sbin/groupadd deploy
  fi

  # Criar usuário deploy se não existir (especificando grupo primário)
  if ! id deploy >/dev/null 2>&1; then
    /usr/sbin/useradd -m -s /bin/bash -g deploy -G sudo deploy
    echo \"deploy:\${senha_deploy}\" | /usr/sbin/chpasswd
  fi

  # Garantir que deploy está no grupo sudo
  /usr/sbin/usermod -aG sudo deploy 2>/dev/null || true
"

#------------------- ETAPA 4.5: Limpar estado inconsistente do Redis ------------
run_quiet "Preparando sistema para Redis..." "bash -lc '
  set -e
  # Verificar se Redis está em estado inconsistente (rc = removed but config remains)
  if dpkg -l 2>/dev/null | grep -q \"^rc.*redis-server\"; then
    echo \"[INFO] Redis em estado inconsistente detectado. Limpando...\"
    # Remover entradas órfãs do statoverride
    dpkg-statoverride --remove /etc/redis/redis.conf 2>/dev/null || true
    dpkg-statoverride --remove /var/log/redis 2>/dev/null || true
    dpkg-statoverride --remove /var/log/redis/redis-server.log 2>/dev/null || true
    dpkg-statoverride --remove /var/lib/redis 2>/dev/null || true
    # Limpar completamente o Redis
    apt-get purge -y redis-server redis-tools 2>/dev/null || true
    apt-get autoremove -y 2>/dev/null || true
    # Remover arquivos de configuração órfãos
    rm -rf /etc/redis 2>/dev/null || true
    rm -rf /var/lib/redis 2>/dev/null || true
    rm -rf /var/log/redis 2>/dev/null || true
    echo \"[INFO] Limpeza do Redis concluída.\"
  else
    echo \"[INFO] Redis não requer limpeza prévia.\"
  fi
'"

#------------------- ETAPA 5: Pacotes essenciais --------------------------------
[ "$TMPFS_ENABLED" = "1" ] && optimize_etapa5_essentials
...
[ "$TMPFS_ENABLED" = "1" ] && optimize_etapa6_postgresql
...
run_quiet "$(t installing) PostgreSQL 17..." "bash -lc '
  ...
  # ============================================================================
  # PRÉ-CHECK: Resolver conflito dpkg-divert do pg_config (IDEMPOTENTE)
  # ============================================================================
  # Contexto: O postgresql-common cria uma diversão de /usr/bin/pg_config para
  # /usr/bin/pg_config.libpq-dev. Se ambos arquivos já existem (de instalação
  # anterior), o dpkg-divert falhará com erro:
  # \"rename involves overwriting '/usr/bin/pg_config.libpq-dev' with different
  #  file '/usr/bin/pg_config', not allowed\"
  #
  # Solução: Detectar conflito e remover arquivos órfãos ANTES da instalação.
  # ============================================================================

  PG_CONFIG=\"/usr/bin/pg_config\"
  PG_CONFIG_LIBPQ=\"/usr/bin/pg_config.libpq-dev\"
  BACKUP_TIMESTAMP=\$(date +%Y%m%d_%H%M%S)

  # Criar diretório de backup se necessário
  mkdir -p /root/backups/pg_config >/dev/null 2>&1 || true

  # Verificar se há conflito potencial
  if [ -f \"\$PG_CONFIG\" ] || [ -f \"\$PG_CONFIG_LIBPQ\" ]; then
    echo \"[PG-DIVERT] Detectados arquivos pg_config existentes, verificando conflito...\" >&2

    # Verificar se há diversão registrada
    if dpkg-divert --list \"\$PG_CONFIG\" | grep -q .; then
      echo \"[PG-DIVERT] Diversão já registrada, removendo para limpeza...\" >&2
      dpkg-divert --remove \"\$PG_CONFIG\" >/dev/null 2>&1 || true
    fi

    # Fazer backup e remover arquivos órfãos
    if [ -f \"\$PG_CONFIG\" ]; then
      echo \"[PG-DIVERT] Backup de pg_config órfão: /root/backups/pg_config/pg_config.\$BACKUP_TIMESTAMP\" >&2
      cp -a \"\$PG_CONFIG\" \"/root/backups/pg_config/pg_config.\$BACKUP_TIMESTAMP\" || true
      rm -f \"\$PG_CONFIG\"
    fi

    if [ -f \"\$PG_CONFIG_LIBPQ\" ]; then
      echo \"[PG-DIVERT] Backup de pg_config.libpq-dev órfão: /root/backups/pg_config/pg_config.libpq-dev.\$BACKUP_TIMESTAMP\" >&2
      cp -a \"\$PG_CONFIG_LIBPQ\" \"/root/backups/pg_config/pg_config.libpq-dev.\$BACKUP_TIMESTAMP\" || true
      rm -f \"\$PG_CONFIG_LIBPQ\"
    fi

    echo \"[PG-DIVERT] Estado limpo, pronto para instalação\" >&2
  else
    echo \"[PG-DIVERT] Nenhum conflito detectado, prosseguindo com instalação\" >&2
  fi
  ...
  # Verificar se cluster 17/main existe, criar se necessário
  if ! pg_lsclusters | grep -q \"17[[:space:]]*main\"; then
    echo \"[INFO] Criando cluster PostgreSQL 17/main...\" >&2
    pg_createcluster 17 main --start
  else
    echo \"[INFO] Cluster 17/main já existe.\" >&2
  fi

  # ============================================================================
  # VALIDAÇÃO PÓS-CRIAÇÃO: Verificar estrutura e permissões do cluster
  # ============================================================================
  PGDATA=\"/var/lib/postgresql/17/main\"

  # Verificar se cluster foi criado em subdiretório incorreto
  if [ -d \"\$PGDATA/main_disk\" ] && [ ! -f \"\$PGDATA/PG_VERSION\" ]; then
    echo \"[WARN] Cluster criado em subdiretório incorreto, corrigindo...\" >&2
    sudo -u postgres bash -c \"
      cd '\$PGDATA'
      mv main_disk/* . 2>/dev/null || true
      mv main_disk/.* . 2>/dev/null || true
      rmdir main_disk 2>/dev/null || rm -rf main_disk
    \"
    echo \"[INFO] Estrutura do cluster corrigida\" >&2
  fi

  # Verificar e corrigir permissões (PostgreSQL requer 700 ou 750)
  CURRENT_PERMS=\$(stat -c '%a' \"\$PGDATA\" 2>/dev/null || echo \"000\")
  if [ \"\$CURRENT_PERMS\" != \"700\" ] && [ \"\$CURRENT_PERMS\" != \"750\" ]; then
    echo \"[WARN] Permissões incorretas (\$CURRENT_PERMS), corrigindo para 700...\" >&2
    chmod 700 \"\$PGDATA\"
  fi

  # Remover PID file obsoleto se existir
  [ -f \"\$PGDATA/postmaster.pid\" ] && rm -f \"\$PGDATA/postmaster.pid\"
  # ============================================================================

  # Habilitar e garantir que serviço está rodando
  systemctl enable postgresql@17-main.service
  systemctl start postgresql@17-main.service || true

  # Aguardar serviço estar pronto (timeout 30s)
  echo \"[INFO] Aguardando PostgreSQL aceitar conexões...\" >&2
  MAX_WAIT=30
  WAITED=0
  until pg_isready -U postgres >/dev/null 2>&1 || [ \$WAITED -ge \$MAX_WAIT ]; do
    sleep 1
    WAITED=\$((WAITED + 1))
  done

  if [ \$WAITED -ge \$MAX_WAIT ]; then
    echo \"[ERRO] PostgreSQL não ficou pronto em \${MAX_WAIT}s\" >&2
    exit 1
  fi

  echo \"[INFO] PostgreSQL pronto após \${WAITED}s\" >&2
'"
...
[ "$TMPFS_ENABLED" = "1" ] && finalize_etapa6_postgresql
...
# Finalizar otimização tmpfs das ETAPAs 5-11
[ "$TMPFS_ENABLED" = "1" ] && finalize_etapa5_11

#------------------- ETAPA 12: Banco PostgreSQL --------------------------------
# Garantir que PostgreSQL está pronto antes de configurar banco
run_quiet "$(t configuring) PostgreSQL (readiness check)..." "bash -lc '
  MAX_WAIT=10
  WAITED=0
  until pg_isready -U postgres >/dev/null 2>&1 || [ \$WAITED -ge \$MAX_WAIT ]; do
    sleep 1
  ...
  if [ \$WAITED -ge \$MAX_WAIT ]; then
    echo \"[WARN] PostgreSQL não respondeu em \${MAX_WAIT}s, tentando continuar...\" >&2
  fi
'"
...
#------------------- ETAPA 13: Código ------------------------------------------
[ "$TMPFS_ENABLED" = "1" ] && optimize_etapa13_code
SOURCE_DIR="$(pwd)/chatia"; DEST_DIR="/home/deploy/${empresa}"
run_quiet "$(t installing) ChatIA code..." "GITHUB_TOKEN='${github_token}' REPO_URL='${repo_url}' GITHUB_BRANCH='main' TMPFS_CLONE_DIR='${CHATIA_TMPFS_CLONE_DIR:-}' bash -lc '
  set -e
  ...
  # Função para verificar espaço/inodes disponíveis
  check_space() {
    local dir=\"\$1\"
    local avail_mb=\$(df -BM \"\$dir\" 2>/dev/null | awk \"NR==2 {gsub(/M/,\\\"\\\",\\\$4); print \\\$4}\")
    local avail_inodes=\$(df -i \"\$dir\" 2>/dev/null | awk \"NR==2 {print \\\$4}\")
    [ -n \"\$avail_mb\" ] && [ \"\$avail_mb\" -gt 500 ] && [ -n \"\$avail_inodes\" ] && [ \"\$avail_inodes\" -gt 10000 ]
  }

  # Selecionar diretório temporário com fallback inteligente
  # Prioridade: 1) tmpfs otimizado, 2) /tmp, 3) /var/tmp, 4) /home/deploy
  if [ -n \"\$TMPFS_CLONE_DIR\" ] && [ -d \"\$TMPFS_CLONE_DIR\" ]; then
    tmpd=\"\$TMPFS_CLONE_DIR/chatia_clone_\$\$\"
    echo \"[INFO] Usando tmpfs otimizado para clone: \$TMPFS_CLONE_DIR\" >&2
  elif check_space \"/tmp\"; then
    tmpd=\"/tmp/chatia_clone_\$\$\"
    echo \"[INFO] Usando /tmp para clone temporário (espaço suficiente)\" >&2
  elif check_space \"/var/tmp\"; then
    tmpd=\"/var/tmp/chatia_clone_\$\$\"
    echo \"[INFO] /tmp com pouco espaço, usando /var/tmp\" >&2
  else
    tmpd=\"/home/deploy/_clone_tmp_\$\$\"
    mkdir -p \"\$tmpd\"
    echo \"[INFO] /tmp e /var/tmp com pouco espaço, usando /home/deploy/_clone_tmp_\$\$\" >&2
  fi

  if [ \"\$GITHUB_TOKEN\" != \"local_install\" ] && [ -n \"\$GITHUB_TOKEN\" ] && [ -n \"\$REPO_URL\" ]; then
    sudo -u deploy mkdir -p \"\$tmpd\"
    tok=\$(urlenc \"\$GITHUB_TOKEN\")
    clean=\${REPO_URL#http://}; clean=\${clean#https://}
    github_url=\"https://\$tok@\${clean}\"

    # Clone otimizado: shallow + sparse-checkout para reduzir I/O e espaço
    echo \"[INFO] Iniciando clone otimizado (shallow + sparse-checkout)...\" >&2
    sudo -u deploy git clone --depth=1 --filter=blob:none --no-checkout --branch \"\$GITHUB_BRANCH\" --single-branch \"\$github_url\" \"\$tmpd\"

    # Configurar sparse-checkout para baixar apenas os diretórios necessários
    cd \"\$tmpd\"
    sudo -u deploy git sparse-checkout init --cone

    # Detectar estrutura do repositório e fazer sparse-checkout apropriado
    if sudo -u deploy git ls-tree -d HEAD | grep -q \"chatia\"; then
      echo \"[INFO] Estrutura detectada: chatia/{backend,frontend}\" >&2
      sudo -u deploy git sparse-checkout set chatia/backend chatia/frontend chatia/ecosystem.config.js
    else
      echo \"[INFO] Estrutura detectada: {backend,frontend} na raiz\" >&2
      sudo -u deploy git sparse-checkout set backend frontend ecosystem.config.js
    fi

    sudo -u deploy git checkout \"\$GITHUB_BRANCH\"
    cd - >/dev/null

    echo \"[INFO] Clone concluído, copiando arquivos para destino final...\" >&2
    mkdir -p \"$DEST_DIR\"

    # Copiar apenas os diretórios necessários usando rsync para eficiência
    if [ -d \"\$tmpd/chatia\" ]; then
      rsync -a --delete \"\$tmpd/chatia/backend/\" \"$DEST_DIR/backend/\"; rsync -a --delete \"\$tmpd/chatia/frontend/\" \"$DEST_DIR/frontend/\"
      [ -f \"\$tmpd/chatia/ecosystem.config.js\" ] && cp \"\$tmpd/chatia/ecosystem.config.js\" \"$DEST_DIR/\"
    elif [ -d \"\$tmpd/backend\" ] && [ -d \"\$tmpd/frontend\" ]; then
      rsync -a --delete \"\$tmpd/backend/\" \"$DEST_DIR/backend/\"; rsync -a --delete \"\$tmpd/frontend/\" \"$DEST_DIR/frontend/\"
      [ -f \"\$tmpd/ecosystem.config.js\" ] && cp \"\$tmpd/ecosystem.config.js\" \"$DEST_DIR/\"
    else
      echo \"[ERROR] Estrutura de diretórios não reconhecida em \$tmpd\" >&2
      exit 9
    fi

    echo \"[INFO] Limpando diretório temporário \$tmpd...\" >&2
    rm -rf \"\$tmpd\"
    echo \"[INFO] Instalação de código concluída com sucesso\" >&2
...
[ "$TMPFS_ENABLED" = "1" ] && finalize_etapa13_code
...
[ "$TMPFS_ENABLED" = "1" ] && optimize_etapa15_backend
...
[ "$TMPFS_ENABLED" = "1" ] && finalize_etapa15_backend
...
run_quiet "$(t configuring) database migrations..." "bash -lc '
  ...
  # Validar que o diretório backend existe
  ...
  # CORREÇÃO: Query atômica para evitar race condition (TOCTOU)
  # Verifica existência e retorna migrations em uma única transação
  APPLIED=\$(sudo -u postgres psql -At -d \"\$DB_NAME\" 2>/dev/null <<'MIGRATIONS_CHECK' || true
...
MIGRATIONS_CHECK
  ...
  if [ -d \"\$MIGRATIONS_DIR\" ]; then
    ...
    if [ \"\$migrations_pending\" = 1 ]; then
      printf \"[INFO] Executando migrations pendentes...\\n\"
      printf \"[INFO] Configurando timeout e variáveis para Sequelize...\\n\"

      # Adicionar timeout e configurações para evitar travamento
      MIGRATION_CMD=\"\$DEPLOY_BASE \\
        export DB_DIALECT=postgres; \\
        export DB_HOST=localhost; \\
        export DB_PORT=5432; \\
        export DB_USERNAME=${empresa}; \\
        export DB_PASSWORD='${senha_deploy}'; \\
        export DB_DATABASE=${empresa}; \\
        export NODE_ENV=production; \\
        timeout --preserve-status 120 ./node_modules/.bin/sequelize db:migrate 2>&1\"

      printf \"[INFO] Executando migrations com timeout de 120 segundos...\\n\"

      # Executar com tratamento de erro
      MIGRATION_OUTPUT=\$(sudo -H -u deploy bash -lc \"\$MIGRATION_CMD\" 2>&1)
      MIGRATION_STATUS=\$?

      if [ \$MIGRATION_STATUS -eq 0 ]; then
        printf \"[SUCCESS] Migrations executadas com sucesso!\\n\"
        echo \"\$MIGRATION_OUTPUT\"
      elif [ \$MIGRATION_STATUS -eq 124 ] || [ \$MIGRATION_STATUS -eq 137 ] || [ \$MIGRATION_STATUS -eq 143 ]; then
        printf \"[ERROR] Timeout ao executar migrations (120s excedidos)\\n\"
        printf \"[INFO] Verificando conexão com banco de dados...\\n\"

        if sudo -u postgres psql -d \"${empresa}\" -c \"SELECT 1\" >/dev/null 2>&1; then
          printf \"[INFO] Banco está acessível. Tentando com timeout maior (300s)...\\n\"

          MIGRATION_CMD_RETRY=\"\$DEPLOY_BASE \\
            export DB_DIALECT=postgres; \\
            export DB_HOST=localhost; \\
            export DB_PORT=5432; \\
            export DB_USERNAME=${empresa}; \\
            ...
            export DB_DATABASE=${empresa}; \\
            export NODE_ENV=production; \\
            timeout --preserve-status 300 ./node_modules/.bin/sequelize db:migrate --debug 2>&1\"

          RETRY_OUTPUT=\$(sudo -H -u deploy bash -lc \"\$MIGRATION_CMD_RETRY\" 2>&1)
          RETRY_STATUS=\$?

          if [ \$RETRY_STATUS -eq 0 ]; then
            printf \"[SUCCESS] Migrations executadas com sucesso na segunda tentativa!\\n\"
            echo \"\$RETRY_OUTPUT\" | tail -n 10
          else
            printf \"[WARNING] Migrations falharam. Pode ser necessário executar manualmente.\\n\"
            printf \"[DEBUG] Últimas linhas do output:\\n\"
            echo \"\$RETRY_OUTPUT\" | tail -n 20
            printf \"\\n[INFO] Para executar manualmente após instalação:\\n\"
            printf \"  cd /home/deploy/${empresa}/backend\\n\"
            printf \"  npx sequelize db:migrate\\n\\n\"
          fi
        else
          printf \"[ERROR] Banco de dados ${empresa} não está acessível\\n\"
          exit 1
        fi
      else
        printf \"[ERROR] Erro ao executar migrations (código: \$MIGRATION_STATUS)\\n\"
        printf \"[DEBUG] Output do erro:\\n\"
        echo \"\$MIGRATION_OUTPUT\" | tail -n 20
        printf \"\\n[INFO] Continuando instalação. Execute migrations manualmente se necessário.\\n\"
      fi
    else
      printf \"[INFO] Nenhuma migration pendente.\\n\"
    fi
  else
    printf \"[INFO] Diretório de migrations não encontrado.\\n\"
  fi
'"

#------------------- ETAPA 16B: Backup SQL (se fornecido) ----------------------
# Nota: optimize_etapa16b_sql deve ser chamado dentro do bloco if que verifica caminho_sql_backup
if [ -n "${sql_backup_path}" ] && [ -f "${sql_backup_path}" ]; then
  run_quiet "$(t importing_backup)..." "DB_NAME='${empresa}' SQL_FILE='${sql_backup_path}' bash -s <<'SQLIMPORT'
set -Euo pipefail

# Definir cores dentro do heredoc
RED='\\033[0;31m'
YELLOW='\\033[1;33m'
NC='\\033[0m'
...
if ! pg_isready -h localhost -p 5432 >/dev/null 2>&1; then
  echo \"[WARN] PostgreSQL não está respondendo. Tentando iniciar...\"

  # Verificar e iniciar o cluster se necessário
  if pg_lsclusters | grep -q \"17[[:space:]]*main[[:space:]]*5432[[:space:]]*down\"; then
    echo \"[INFO] Iniciando cluster PostgreSQL 17/main...\"
    sudo pg_ctlcluster 17 main start || {
      echo \"[ERROR] Falha ao iniciar PostgreSQL\"
      exit 1
    }
    # Aguardar PostgreSQL estar pronto
    for i in {1..30}; do
      if pg_isready -h localhost -p 5432 >/dev/null 2>&1; then
        echo \"[INFO] PostgreSQL iniciado com sucesso\"
        break
      fi
      echo \"[INFO] Aguardando PostgreSQL... (\$i/30)\"
      sleep 1
    done
  fi
  ...
  # Verificação final
  if ! pg_isready -h localhost -p 5432 >/dev/null 2>&1; then
    printf \"\${RED}[ERROR] PostgreSQL não está disponível\${NC}\\n\"
    exit 1
  fi
fi

echo \"[INFO] Iniciando import SQL (DB=\$DB_NAME, FILE=\$SQL_FILE)\"

if [ -z \"\${SQL_FILE:-}\" ] || [ ! -f \"\$SQL_FILE\" ]; then
  printf \"\${RED}[ERROR] Arquivo SQL inválido ou inexistente: %s\${NC}\\n\" \"\$SQL_FILE\"
  exit 1
fi

# Validar comandos proibidos no backup (\\restrict / \\unrestrict)
# (usar double-quotes para evitar conflito de aspas no caller)
if grep -qiE \"^[[:space:]]*\\\\(restrict|unrestrict)\" \"\$SQL_FILE\"; then
  printf \"\${RED}[ERROR] Backup contém comandos inválidos (\\\\restrict ou \\\\unrestrict)!\${NC}\\n\"
  printf \"\${YELLOW}[FIX] Execute: sed -i '/^[[:space:]]*\\\\\\\\(restrict\\|unrestrict\\)/Id' '\$SQL_FILE'\${NC}\\n\"
  exit 1
fi

# Extrair OWNER do backup (primeira ocorrência)
BACKUP_OWNER=\"\"
if BO=\$(grep -m 1 \"OWNER TO\" \"\$SQL_FILE\" | sed \"s/.*OWNER TO \\([^;]*\\);/\\1/\" | tr -d \" \\n\" 2>/dev/null); then
  BACKUP_OWNER=\"\$BO\"
fi

# Limpar schema public para import limpo
printf \"[INFO] Limpando schema public...\\n\"
sudo -u postgres psql -d \"\$DB_NAME\" <<CLEANSQL
...
printf \"[INFO] Ajustes de role do backup (se necessários)...\\n\"
if [ -n \"\$BACKUP_OWNER\" ] && [ \"\$BACKUP_OWNER\" != \"\$DB_NAME\" ]; then
  TEMP_PASSWORD=\$(openssl rand -base64 32 2>/dev/null || head -c 32 /dev/urandom | base64)
  sudo -u postgres psql -c \"DO \\\$\\\$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='\$BACKUP_OWNER') THEN CREATE ROLE \\\"\$BACKUP_OWNER\\\" WITH LOGIN PASSWORD '\$TEMP_PASSWORD'; END IF; END \\\$\\\$;\"
  sudo -u postgres psql -c \"GRANT ALL PRIVILEGES ON DATABASE \\\"\$DB_NAME\\\" TO \\\"\$BACKUP_OWNER\\\";\" || true
  sudo -u postgres psql -d \"\$DB_NAME\" -c \"GRANT ALL ON SCHEMA public TO \\\"\$BACKUP_OWNER\\\";\" || true
fi

printf \"[INFO] Importando backup (timeout 30m)...\\n\"
if ! timeout 1800 sudo -u postgres psql -d \"\$DB_NAME\" < \"\$SQL_FILE\" 2>&1 | tee -a \"\${LOG_FILE:-/dev/null}\" | tail -n 20; then
  ec=\${PIPESTATUS[0]}
  if [ \"\$ec\" = \"124\" ]; then
    printf \"\${RED}[ERROR] Importação excedeu timeout de 30 minutos\${NC}\\n\"
  else
    printf \"\${RED}[ERROR] Falha na importação do backup SQL (exit code: \$ec)\${NC}\\n\"
  fi
  exit 1
fi

printf \"[INFO] Verificando encerramento de transações...\\n\"
for i in {1..10}; do
  ACTIVE=\$(sudo -u postgres psql -At -d \"\$DB_NAME\" -c \"SELECT count(*) FROM pg_stat_activity WHERE datname='\$DB_NAME' AND state='active' AND query NOT LIKE '%pg_stat_activity%';\" 2>/dev/null || echo \"0\")
  [ \"\$ACTIVE\" = \"0\" ] && break
  printf \"[INFO] Aguardando \$ACTIVE conexões ativas... (\$i/10)\\n\"
  sleep 1
done

printf \"[INFO] Ajustando ownership via \\\\gexec...\\n\"
sudo -u postgres psql -d \"\$DB_NAME\" <<EOF
SELECT 'ALTER TABLE public.\"' || tablename || '\" OWNER TO ${empresa};'
FROM pg_tables WHERE schemaname = 'public' \\gexec
SELECT 'ALTER SEQUENCE public.\"' || sequence_name || '\" OWNER TO ${empresa};'
FROM information_schema.sequences WHERE sequence_schema = 'public' \\gexec
EOF

printf \"[INFO] Correções de schema compatíveis...\\n\"
sudo -u postgres psql -d \"\$DB_NAME\" <<SQLFIX2
DO \\\$\\\$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE \"Companies\" ADD COLUMN timezone VARCHAR(255) DEFAULT 'America/Sao_Paulo';
  END IF;
END \\\$\\\$;
DO \\\$\\\$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='CompaniesSettings' AND column_name='overrideDefaultTimezone'
  ) THEN
    ALTER TABLE \"CompaniesSettings\" ADD COLUMN \"overrideDefaultTimezone\" BOOLEAN DEFAULT false;
  END IF;
END \\\$\\\$;
SQLFIX2

# Limpeza de role temporária, se criada
if [ -n \"\$BACKUP_OWNER\" ] && [ \"\$BACKUP_OWNER\" != \"\$DB_NAME\" ]; then
  # Primeiro, reassign todos os objetos do backup owner para o owner correto
  sudo -u postgres psql -d \"\$DB_NAME\" -c \"REASSIGN OWNED BY \\\"\$BACKUP_OWNER\\\" TO \\\"\$DB_NAME\\\";\" 2>/dev/null || true
  # Remover privilégios da role antes de dropar
  sudo -u postgres psql -d \"\$DB_NAME\" -c \"DROP OWNED BY \\\"\$BACKUP_OWNER\\\";\" 2>/dev/null || true
  # Agora pode dropar a role
  sudo -u postgres psql -c \"DROP ROLE IF EXISTS \\\"\$BACKUP_OWNER\\\";\" 2>/dev/null || true
fi

printf \"[INFO] Backup SQL processado.\\n\"
SQLIMPORT"

  # Atualizar credenciais do admin (executado FORA de run_quiet para evitar problemas de escape)
  printf "\r\033[K${GRAY_LIGHT}• $(t updating_admin)...${NC}\n" | tee -a "$LOG_FILE"
  (
    set +e  # Não parar em erros
    BACKEND_DIR="/home/deploy/${empresa}/backend"
    DB_NAME="${empresa}"
    ADMIN_EMAIL="${email_deploy}"
    ADMIN_PASSWORD="${senha_master}"

    cd "$BACKEND_DIR" || exit 1

    # Verificar se bcryptjs está instalado
    if [ ! -d "node_modules/bcryptjs" ]; then
      echo "[WARN] bcryptjs não encontrado. Pulando atualização de credenciais."
      exit 0
    fi

    # Criar script de hash DENTRO do backend (para acessar node_modules via require)
    HASH_SCRIPT="$BACKEND_DIR/hash_pw_temp_$$.js"
...
HASHEOF
    ...
    # Gerar hash (executar do diretório correto como usuário deploy)
    HASHED_PW=$(sudo -H -u deploy bash -lc "cd '$BACKEND_DIR' && node hash_pw_temp_$$.js '$ADMIN_PASSWORD'" 2>&1)
    rm -f "$HASH_SCRIPT"

    # Verificar hash (formato bcrypt: $2a$, $2b$ ou $2y$)
    if [ -z "$HASHED_PW" ] || ! echo "$HASHED_PW" | grep -qE '^\$2[aby]\$'; then
      echo "[WARN] Não foi possível gerar hash bcrypt. Pulando atualização."
      echo "[DEBUG] Hash gerado: $HASHED_PW"
      exit 0
    fi

    echo "[INFO] Hash bcrypt gerado com sucesso."

    # Escapar aspas para PostgreSQL
    EMAIL_ESC=$(printf "%s" "$ADMIN_EMAIL" | sed "s/'/''/g")
    PASS_ESC=$(printf "%s" "$HASHED_PW" | sed "s/'/''/g")

    # Atualizar via psql (verificar se usuário existe primeiro)
    USER_EXISTS=$(sudo -u postgres psql -At -d "$DB_NAME" -c "SELECT COUNT(*) FROM \"Users\" WHERE id = 1;" 2>/dev/null || echo "0")

    if [ "$USER_EXISTS" = "1" ]; then
      # Admin existe: atualizar
      sudo -u postgres psql -d "$DB_NAME" -c "UPDATE \"Users\" SET email = '$EMAIL_ESC', \"passwordHash\" = '$PASS_ESC', super = true WHERE id = 1;" 2>&1 || echo "[WARN] Falha ao atualizar credenciais"
      printf "[INFO] Credenciais do admin atualizadas: %s\n" "$ADMIN_EMAIL"
    else
      # Admin não existe: criar novo
      printf "[INFO] Admin não encontrado. Criando novo usuário admin...\n"

      # Obter primeiro companyId disponível
      COMPANY_ID=$(sudo -u postgres psql -At -d "$DB_NAME" -c "SELECT id FROM \"Companies\" ORDER BY id LIMIT 1;" 2>/dev/null || echo "1")

      sudo -u postgres psql -d "$DB_NAME" <<ADMINSQL
INSERT INTO "Users" (
  name, email, "passwordHash", profile, "tokenVersion",
  "companyId", super, "createdAt", "updatedAt"
) VALUES (
  'Admin',
  '$EMAIL_ESC',
  '$PASS_ESC',
...
ADMINSQL
      printf "[INFO] Admin criado com sucesso: %s\n" "$ADMIN_EMAIL"
    fi
  ) >> "$LOG_FILE" 2>&1 || true
  log_success "$(t done_ok)"
  progress_inc
...
#------------------- ETAPA 16C: Seeds ------------------------------------------
# Se backup SQL foi fornecido, pular seeds (dados já foram importados)
if [ -n "${sql_backup_path}" ] && [ -f "${sql_backup_path}" ]; then
  run_quiet "$(t configuring) database seeds..." "bash -lc '
    printf \"[INFO] Backup SQL foi importado. Pulando seeds (dados já existem no banco).\\n\"
  '"
else
  run_quiet "$(t configuring) database seeds..." "bash -lc '
    set -Eu  # Tolerante a erros
    BACKEND_DIR=\"/home/deploy/${empresa}/backend\"
    SEEDS_DIR=\"\$BACKEND_DIR/dist/database/seeds\"
    SENTINEL=\"\$BACKEND_DIR/.chatia_seeds.fingerprint\"

    DEPLOY_BASE=\"cd \\\"\$BACKEND_DIR\\\"; export PNPM_HOME=\\\$HOME/.local/share/pnpm; export PATH=\\\"\\\$PNPM_HOME:\\\\\$PATH\\\";\"

    # Calcular hash dos seeds
    # CORREÇÃO: Hash baseado no conteúdo concatenado (não hash de hashes)
    if [ -d \"\$SEEDS_DIR\" ] && find \"\$SEEDS_DIR\" -name \"*.js\" -print -quit 2>/dev/null | grep -q .; then
      HASH=\$(find \"\$SEEDS_DIR\" -type f -name \"*.js\" -print0 2>/dev/null | LC_ALL=C sort -z | xargs -0 cat 2>/dev/null | sha256sum | cut -d\" \" -f1)
    else
      HASH=\"0\"
    fi

    PREV=\$(cat \"\$SENTINEL\" 2>/dev/null || echo \"0\")

    if [ \"\$HASH\" = \"0\" ]; then
      printf \"[INFO] Nenhum seed encontrado.\\n\"
    elif [ \"\$HASH\" = \"\$PREV\" ]; then
      printf \"[INFO] Seeds já aplicados (hash inalterado).\\n\"
    else
      printf \"[INFO] Executando seeds (tolerante a erros de dados duplicados)...\\n\"
      # Executar seeds com timeout mas tolerar erros (dados podem já existir do backup)
      SEEDS_CMD=\"\$DEPLOY_BASE \\
        export DB_DIALECT=postgres; \\
        export DB_HOST=localhost; \\
        export DB_PORT=5432; \\
        export DB_USERNAME=${empresa}; \\
        ...
        export DB_DATABASE=${empresa}; \\
        export NODE_ENV=production; \\
        timeout --preserve-status 120 ./node_modules/.bin/sequelize db:seed:all 2>&1\"

      sudo -H -u deploy bash -lc \"\$SEEDS_CMD\" || echo \"[WARN] Alguns seeds falharam ou timeout (pode ser normal se dados já existem)\"
      printf \"%s\" \"\$HASH\" > \"\$SENTINEL\"
      chown deploy:deploy \"\$SENTINEL\" 2>/dev/null || true
    fi
  '"
fi
...
run_quiet "$(t configuring) company name..." "bash -lc '
  ...
  # CORREÇÃO: Escape completo - aspas simples E backslashes
  SAFE_NEW_NAME=\$(printf \"%s\" \"\$TARGET_NAME\" | sed -e \"s/'\''/'\'''\'''\''/g\" -e \"s/\\\\\\\\/\\\\\\\\\\\\\\\\/g\")
  ...
  if [ \"\$CURRENT_NAME\" = \"\$TARGET_NAME\" ]; then
    ...
    # Usar printf para construir o comando SQL de forma segura
    printf \"UPDATE \\\"Companies\\\" SET name = '%s' WHERE id = 1;\" \"\$SAFE_NEW_NAME\" | sudo -u postgres psql -d \"\$DB_NAME\" >/dev/null
  fi
'"
...
[ "$TMPFS_ENABLED" = "1" ] && optimize_etapa18_frontend
...
[ "$TMPFS_ENABLED" = "1" ] && finalize_etapa18_frontend

#------------------- ETAPA 19: Builds ------------------------------------------
[ "$TMPFS_ENABLED" = "1" ] && optimize_etapa19_builds
...
[ "$TMPFS_ENABLED" = "1" ] && finalize_etapa19_builds
...
run_quiet "$(t configuring) PM2 apps..." "bash -lc '
  ...
  if [ -f \"\$PM2_CONFIG\" ]; then
    if sudo -H -u deploy bash -lc \"\$DEPLOY_ENV cd '/home/deploy/${empresa}'; pm2 delete ecosystem.config.js 2>/dev/null || true; pm2 start ecosystem.config.js --update-env\"; then
      printf \"[INFO] PM2 iniciado com ecosystem.config.js.\\n\"
      reload_success=1
    else
      printf \"[WARN] PM2 start falhou; aplicando fallback com start manual.\\n\"
    fi
  else
...
#------------------- ETAPA 21: Nginx/SSL ---------------------------------------
[ "$TMPFS_ENABLED" = "1" ] && optimize_etapa21_certbot
...
[ "$TMPFS_ENABLED" = "1" ] && finalize_etapa21_certbot
...
# Usar variáveis dinâmicas coletadas no wizard (com fallback seguro)
PRINT_EMAIL="${email_deploy:-${ADMIN_EMAIL:-N/A}}"
PRINT_PASS="${senha_master:-${ADMIN_PASSWORD:-N/A}}"
echo "  $(t email): ${PRINT_EMAIL}"
echo "  $(t password): ${PRINT_PASS}"
echo ""
echo "==============================================================================="
echo ""
echo "✅ $(t services_up)"
